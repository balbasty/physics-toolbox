function varargout = lib(varargin)
...
% ----------------
%
% logp = utils.gmm.lib('Marginal', X, {MU,A},   const, {C, L}, E)
% logp = utils.gmm.lib('Marginal', X, {MU,V,n}, const, {C, L}, E)
% > Observation's marginal log probability within each cluster
%
% const = utils.gmm.lib('Const',  MU,     A,    (L))
% const = utils.gmm.lib('Const', {MU,b}, {V,n}, (L))
...
%
% FORMAT [SS0,SS1,SS2] = utils.gmm.lib('SuffStat', X, Z, W)
% FORMAT [SS0,SS1,SS2] = utils.gmm.lib('SuffStat', 'base',  X, Z, W)
% FORMAT [SS0,SS1,SS2] = utils.gmm.lib('SuffStat', 'infer', SS0, SS1, SS2, {MU,A}, L)
% FORMAT         [SS2] = utils.gmm.lib('SuffStat', 'bin',   E, Z, W, {C,L})
...
%
% [MU,A]       = utils.gmm.lib('UpdateClusters', SS0, SS1, SS2)
% [MU,A,b,V,n] = utils.gmm.lib('UpdateClusters', SS0, SS1, SS2, {MU0,b0,V0,n0})
...
%
% [GaussPrior,extras] = utils.gmm.lib('updatehyperpars',cluster,GaussPrior,varargin)
% > Update VB-GMM hyper-parameters (MU,b,V,n)
%
% X = utils.gmm.lib('InferMissing', X, Z, {MU,A}, {C,L})
...
% ---------------------
%
% [lb,const] = utils.gmm.lib('MarginalSum', SS0, SS1, SS2,  MU,     A,    L, SS2b)
% [lb,const] = utils.gmm.lib('MarginalSum', SS0, SS1, SS2, {MU,b}, {V,n}, L, SS2b)
% > Compute conditional datasum: E[ln p(g|MU,A,Z)]
%   Also returns the result of utils.gmm.lib('const')
%
% [klMU,klA] = utils.gmm.lib('KL', 'GaussWishart', {MU,b}, {V,n}, {MU0,b0}, {V0,n0})
% > KL divergence between two Gauss-Wishart distributions
%
% klP = utils.gmm.lib('KL', 'Dirichlet', a, a0)
% > KL divergence between two Dirichlet distributions
%
% klZ = utils.gmm.lib('KL', 'Categorical', Z, W, logPI)
...
% > Plot mixture fit
%
% utils.gmm.lib('plot', 'cat', dm, Z, Template, (wintitle))
% > Plot (categorical) responsibilities and template (if available)
%
% utils.gmm.lib('plot', 'gaussprior', GaussPrior, (wintitle))
% > Plot VB-GMM hyper-parameters
%
% c = utils.gmm.lib('plot', 'cat2rgb', f, pal)
...
% -------------
%
% gmm = utils.gmm.lib('extras', 'more_gmms', gmm, part)
...
if nargin == 0
    help utils.gmm.lib
    error('Not enough argument. Type ''help utils.gmm.lib'' for help.');
end
id = varargin{1};
...
switch lower(id)
    ...
    otherwise
        help utils.gmm.lib
        error('Unknown function %s. Type ''help utils.gmm.lib'' for help.', id)
end
...
% FORMAT X = utils.gmm.lib('missing', X, Z, {MU,A}, {C,L})
% X  - NxP   observations
...
% FORMAT Z = utils.gmm.lib('responsibility', logpX, logPI)
%
...
% FORMAT [SS0,SS1,SS2] = utils.gmm.lib('suffstat', X, Z, W)
% FORMAT [SS0,SS1,SS2] = utils.gmm.lib('suffstat', 'base',  X, Z, W, {C,L})
% FORMAT [SS0,SS1,SS2] = utils.gmm.lib('suffstat', 'infer', SS0, SS1, SS2, {MU,A}, L)
% FORMAT         [SS2] = utils.gmm.lib('suffstat', 'bin',   E, Z, W, {C,L})
%
...
if nargin == 0
    help utils.gmm.lib>suffstat
    error('Not enough argument. Type ''help utils.gmm.lib>suffstat'' for help.');
end
...
switch lower(id)
    ...
    otherwise
        help utils.gmm.lib>suffstat
        error('Unknown function %s. Type ''help utils.gmm.lib>suffstat'' for help.', id)
end
...
% FORMAT c = utils.gmm.lib('const', {MU,b}, {V,n})
% FORMAT c = utils.gmm.lib('const', {MU,b}, {A})
% FORMAT c = utils.gmm.lib('const', {MU},   {A})
% FORMAT c = utils.gmm.lib('const', ..., L)
% MU - (Expected) mean
...
if verbose
    % Visualise results
    utils.gmm.lib('plot','gaussprior',GaussPrior,figname);
end
% =========================================================================
...
if nargin == 0
    help utils.gmm.lib>kl
    error('Not enough argument. Type ''help utils.gmm.lib>kl'' for help.');
end
id = varargin{1};
varargin = varargin(2:end);
switch lower(id)
    ...
    otherwise
        help utils.gmm.lib>kl
        error('Unknown function %s. Type ''help utils.gmm.lib>kl'' for help.', id)
end
...
% FORMAT code = utils.gmm.lib('obs2code', X)
%
...
% FORMAT L = utils.gmm.lib('double2int', L)
%
...
% FORMAT type = utils.gmm.lib('range2int', maxval, minval)
%
...
%
% utils.gmm.lib('plot', 'lb', lb, (wintitle))
% > Plot lower bound
%
% utils.gmm.lib('plot', 'gmm', {X,W}, {MU,A}, PI, (wintitle))
% > Plot mixture fit
%
% utils.gmm.lib('plot', 'cat', dm, Z, Template, (wintitle))
% > Plot (categorical) responsibilities and template (if available)
...
if nargin == 0
    help utils.gmm.lib>plot
    error('Not enough argument. Type ''help utils.gmm.lib>plot'' for help.');
end
id = varargin{1};
varargin = varargin(2:end);
switch lower(id)
    ...
    otherwise
        help utils.gmm.lib>plot
        error('Unknown function %s. Type ''help utils.gmm.lib>plot'' for help.', id)
end
...
c = utils.gmm.lib('plot', 'cat2rgb', Z, colors);
c = squeeze(c(:,:,:,:));
c = permute(c,[2 1 3]);
...
if do_subpl
    ...
    c = utils.gmm.lib('plot', 'cat2rgb', Template, colors);
    c = squeeze(c(:,:,:,:));
    c = permute(c,[2 1 3]);
    imagesc(c); axis image off xy;   
...
switch lower(id)
    ...
    otherwise
        help utils.gmm.lib>extras
        error('Unknown function %s. Type ''help utils.gmm.lib>extras'' for help.', id)
end
% =========================================================================

% =========================================================================
function gmm = more_gmms(gmm,part)
% FORMAT gmm = utils.gmm.lib('extras', 'more_gmms', gmm, part)
%
