...
%
% help utils.gmm.fit>Options
% help utils.gmm.fit>TellMeMore
%__________________________________________________________________________
...
if ischar(X) && strcmpi(X, 'apply')
    [varargout{1:nargout}] = utils.gmm.apply(varargin{:});
    return
end
...
if Missing
    ...
    code      = utils.gmm.lib('obs2code', X);     % Code image
    code_list = unique(code);                   % List of codes
    missmsk   = [];
else
    % Compute mask of removed rows
    missmsk   = any(isnan(X),2);
    code      = utils.gmm.lib('double2int', (2^P-1) * ones(sum(~missmsk),1));
    code_list = 2^P-1;
end
...
% -------------------------------------------------------------------------
% MAIN LOOP
[Z,cluster,prop] = utils.gmm.loop({X,W}, {mean,prec}, ...
...
if nargout >= 9 && Missing
    X = utils.gmm.lib('InferMissing', X, Z, {MU,A}, {code,code_list});
end
