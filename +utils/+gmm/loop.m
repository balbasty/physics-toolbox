...
% spm_gmm instead.
%
% FORMAT [resp,cluster,prop,lb] = utils.gmm.loop(obs,cluster,prop,...)
%
% MANDATORY
% ---------
%
...
if ~iscell(cluster) || numel(cluster) < 2
    error('[utils.gmm.loop] At least one mean and one precision matrix are needed.');
else
    if ~iscell(cluster{1})
        MU = cluster{1};
    else
