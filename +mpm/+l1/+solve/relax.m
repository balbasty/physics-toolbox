function dy = relax(H, g, w, mode, prec, vs, opt)
...
% d    - {nx ny nz nf}         - Step: d = H\g
    ...
    wbnd = double(max(w(:)));
    prec_bound = prec;
    prec_bound(mode==1) = wbnd * prec_bound(mode==1).^2;
    ...
    % Forward operator: A = E + F
        ...
        if nargin < 2
            ...
            Ax(:,:,:,mode==1) = spm_field('vel2mom1', single(x(:,:,:,mode==1)), single(w), [vs   1  ], prec(mode==1).^2);
            Ax(:,:,:,mode==2) = spm_field('vel2mom',  single(x(:,:,:,mode==2)),            [vs 0 1 0], prec(mode==2).^2);
    ...
    % Forward operator: inv(E)
        spm_field('boundary', 1);
        if nargin < 2
            x = spm_field(H, x, [1 1 1 scl 0 0], prec.^2);
        else
            Hsub = reshape(H, [], size(H,4));
            Hsub = Hsub(ind,:);
            Hsub = reshape(Hsub, [], 1, 1, size(Hsub,2));
            ...
            xsub = xsub(ind,:);
            xsub = reshape(xsub, [], 1, 1, size(xsub,2));
            x    = spm_field(Hsub, xsub, [1 1 1 scl 0 0], prec.^2);
