...
if isfield(pre, 'B1m')
    ...
    B1m = utils.pull(single(B1m.map()), (T\B1m.mat)\estatics.PDw.mat, estatics.dim);
...
if isfield(pre, 'B1p')
    ...
    B1p = utils.pull(single(B1p.map()), (T\B1p.mat)\estatics.PDw.mat, estatics.dim);
...
if isfield(pre, 'B1m')
    ...
    B1m = utils.pull(single(B1m.map()), (T\B1m.mat)\estatics.T1w.mat, estatics.dim);
    B1m = log(max(B1m, eps('single')));
end
fprintf('Read T1w\n');
...
if isfield(pre, 'B1p')
    ...
    if strcmpi(B1p.unit, 'p.u.'), norm = 100; else, norm = 1; end
    B1p = utils.pull(single(B1p.map()), (T\B1p.mat)\estatics.T1w.mat, estatics.dim);
    B1p = max(B1p/norm, eps('single'));
end
FA_T1w = FA_T1w .* B1p;
clear B1p
...
if isfield(estatics, 'MTw')
    ...
    if isfield(pre, 'B1m')
        ...
        B1m = utils.pull(single(B1m.map()), (T\B1m.mat)\estatics.MTw.mat, estatics.dim);
        B1m = log(max(B1m, eps('single')));
    end
    fprintf('Read MTw\n');
    ...
    if isfield(pre, 'B1p')
        ...
        if strcmpi(B1p.unit, 'p.u.'), norm = 100; else, norm = 1; end
        B1p = utils.pull(single(B1p.map()), (T\B1p.mat)\estatics.MTw.mat, estatics.dim);
        B1p = max(B1p/norm, eps('single'));
    end
    FA_MTw = FA_MTw * B1p;
    clear B1p
